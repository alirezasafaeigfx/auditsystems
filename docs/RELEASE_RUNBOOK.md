...
Before dispatching a real deployment, run `.github/workflows/production-ssh-preflight.yml` from `main`. The preflight is manual-only, uses the protected `production` Environment on an ephemeral GitHub-hosted runner, does not checkout repository code, and performs no backup, migration, upload, process restart, symlink change, or deploy. It only validates the protected SSH inputs, pins the scanned ED25519 host key to `VPS_HOST_KEY_SHA256`, proves the private key is parseable, connects with strict host verification, and checks that `VPS_BASE_DIR` exists. The deployment workflow uses the same hosted-runner trust boundary. A non-successful preflight is a production blocker.
