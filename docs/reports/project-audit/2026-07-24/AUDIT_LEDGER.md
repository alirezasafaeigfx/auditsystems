...
| Surface consistency | PASS_SYNTHETIC | Synthetic entry-point tests cover FA/EN report rendering, machine-readable report API output, comparison deltas, direct PDF generation, notifications, trends, and monthly Markdown/PDF aggregates. Missing categories are unavailable rather than `100`; failed and invalid records expose no numeric score. |
...
| Boundary | Verdict | Evidence |
|---|---|---|
| Repository implementation | CANDIDATE | PR [#17](https://github.com/alirezasafaeigfx/auditsystems/pull/17) extracts the daemon lease lifecycle into a reusable worker cycle, accepts the daemon shutdown signal before new leases, and fail-closes the destructive PostgreSQL suite unless it targets the declared loopback test database. Queue ownership, heartbeat, timeout, retry, and terminal-failure policy remain unchanged. |
